...
async function run() {
  ...
  const quiz = await Assessment.create({
    ...
    dueDate: new Date(),
    questions: [
      { prompt: 'What does ETL stand for?', type: 'MCQ', options: ['Extract Transform Load', 'Enter Test Learn'], correctIndex: 0, weight: 1 }
    ]
  });
  ...
  const assess = await Assessment.create({
    ...
    dueDate: new Date(),
    questions: [
      { prompt: 'Primary key usage?', type: 'TrueFalse', options: ['True','False'], correctIndex: 0, weight: 2 }
    ]
  });
  ...
  const assignment = await Assessment.create({
    ...
    dueDate: new Date(),
    questions: [
      { prompt: 'Design a simple ETL pipeline with schema.', type: 'Short', options: [], correctIndex: 0, weight: 3 }
    ]
  });
